...
if($LASTEXITCODE -eq 0)
{
    ...
    if($LASTEXITCODE -eq 0)
    {
        ...
        $result = dotnet publish .\AirZapto.WebServices.csproj -c release /p:PublishProfile=.\Properties\PublishProfiles\FolderProfile.pubxml
        if($LASTEXITCODE -eq 0)
        {
            # Success
            Write-Output "Publish Success"
            cd C:\Users\mbillet.NOVACATH\Logiciel\Zapto\AirZapto.WebServices\bin\release\net6.0\linux-arm\;
